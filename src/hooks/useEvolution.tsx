...
} from '../interfaces/evolutionChainInterface';
...
const useEvolution = (id: number) => {
  ...
  const getEveeEvolutions = async (evolutionChain: Chain) => {
    ...
    evolutionChain?.evolves_to.map(x =>
      eveeEvolutionChain.push(x.species.name),
    );
    let eveeEvolutionsNamesAndPictures: evolutionEveeProps[] = [];
    Promise.all(
      eveeEvolutionChain.map(async name => {
        const eveeEvolutionDetailResp = await pokemonApi.get<PokemonDetail>(
  ...
  const getEvolutionData = (pokemonEvolutionChain: string[]) => {
    ...
    const firstEvolution = pokemonList.find(
      pokemon => pokemon.name === pokemonEvolutionChain[0],
    );
    pokemonEvolution.push(firstEvolution);
    const secondEvolution = pokemonList.find(
      pokemon => pokemon.name === pokemonEvolutionChain[1],
    );
    pokemonEvolution.push(secondEvolution);
    const thirdEvolution = pokemonList.find(
      pokemon => pokemon.name === pokemonEvolutionChain[2],
    );
